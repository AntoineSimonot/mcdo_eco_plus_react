...
function ShoppingIngredientButton({product_data, product} ) {
    ...
    useEffect(() => {
            setBtnText(product.excludedIngredients.some(data => data.ingredient.id === product_data.ingredient.id) ? "Add" : "Remove")
    }, [shoppingCart]);
    ...
    const newExcludedIngredients = (newExcludedIngredients) => { shoppingCartUpdated(newExcludedIngredients) }

    const updateExcludedIngredient = (product_data) => {
        
        if (product.excludedIngredients.some(data => data.ingredient.id === product_data.ingredient.id)) {
            newExcludedIngredients(product.excludedIngredients.filter(data => data.ingredient.id !== product_data.ingredient.id))
        }
        else{
